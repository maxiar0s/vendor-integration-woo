...
jQuery(document).ready( function () {
  ...
  jQuery("#fDownLoadCSVWooCatalogo").click(function (e) {
    e.preventDefault();
    jQuery("#fDownLoadCSVWooCatalogo").html('<i class="fa fa-spinner fa-spin" style="font-size:20px"></i>').addClass('disabled');

    // Realizar la solicitud AJAX para obtener el archivo CSV
    var url = Global.url + "?action=" + aDownLoadCSVWooCatalogo.action + "&nonce=" + Global.nonce;
  ...
  jQuery("#fActualizarWooCatalogoJson").click(function (e) {
    e.preventDefault();
      jQuery("#fActualizarWooCatalogoJson").html('<i class="fa fa-spinner fa-spin" style="font-size:20px"></i>').addClass('disabled');
      jQuery.ajax({
        type:"POST",
        url: Global.url,
        data:{
  ...
  jQuery("#fUpdateStockWooCatalogo").click(function (e) {
    e.preventDefault();
    jQuery("#fUpdateStockWooCatalogo").html('<i class="fa fa-spinner fa-spin" style="font-size:20px"></i>').addClass('disabled');
    jQuery.ajax({
      type:"POST",
      url: Global.url,
      data:{
  ...
  jQuery("#fUpdatePrecioWooCatalogo").click(function (e) {
    e.preventDefault();
    jQuery("#fUpdatePrecioWooCatalogo").html('<i class="fa fa-spinner fa-spin" style="font-size:20px"></i>').addClass('disabled');
    jQuery.ajax({
      type:"POST",
      url: Global.url,
      data:{
    ...
    success: function(data) {
        ...
        if (objCatalogWooCatalogo.data.length > 0) {
            ...
            jQuery('.view-config').html(
              ...
              +'    <td class="tg-0lax tgwoocatalogo">'+objCatalogWooCatalogo.data[0].part_number+'</td>'
              +'  </tr>'
              +'  <tr>'
              +'    <td class="tg-0lax tgwoocatalogo">Precio más bajo</td>'
              +'    <td class="tg-0lax tgwoocatalogo">'+objCatalogWooCatalogo.data[0].PBP+': '+objCatalogWooCatalogo.data[0].precioMasBajo+' USD</td>'
              +'  </tr>'
              +'  <tr>'
              +'    <td class="tg-0lax tgwoocatalogo">Precio más alto</td>'
              +'    <td class="tg-0lax tgwoocatalogo">'+objCatalogWooCatalogo.data[0].PAP+': '+objCatalogWooCatalogo.data[0].precioMasAlto+' USD</td>'
              +'  </tr>'
              +'  <tr>'
              +'    <td class="tg-0lax tgwoocatalogo">Proveedores</td>'
              +'    <td class="tg-0lax tgwoocatalogo">'+objCatalogWooCatalogo.data[0].proveedores+'</td>'
              +'  </tr>'
              +'</tbody>'
              +'</table>'
            );
            ...
            jQuery('.popup-overlay').fadeOut('slow');
            jQuery('.cd-popup').addClass('is-visible');
            jQuery('.view-config').html("No se encontraron datos en el array 'data'.");
        }
    ...
    success: function(data) {
        ...
        if (objCatalogWooCatalogo.data.length > 0) {
            ...
            jQuery('.view-config').html(
              ...
              +'    <td class="tg-0lax tgwoocatalogo">'+objCatalogWooCatalogo.data[0].proveedorStockMasBajo+': '+objCatalogWooCatalogo.data[0].stockMasBajo+'</td>'
              +'  </tr>'
              +'  <tr>'
              +'    <td class="tg-0lax tgwoocatalogo">Stock más alto</td>'
              +'    <td class="tg-0lax tgwoocatalogo">'+objCatalogWooCatalogo.data[0].proveedorStockMasAlto+': '+objCatalogWooCatalogo.data[0].stockMasAlto+'</td>'
              +'  </tr>'
              +'  <tr>'
              +'    <td class="tg-0lax tgwoocatalogo">Proveedores</td>'
              +'    <td class="tg-0lax tgwoocatalogo">'+objCatalogWooCatalogo.data[0].proveedores+'</td>'
