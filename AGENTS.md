...
| Action | Skill |
|--------|-------|
| Creating or modifying WordPress plugin hooks, AJAX handlers, SQL, settings pages, custom post types, or REST endpoints | `wordpress-plugin-core` |
| Fixing plugin security issues (SQL injection, XSS, CSRF, nonce/capability checks, wpdb::prepare usage) | `wordpress-plugin-core` |
| Troubleshooting WordPress 6.8+ bcrypt migration or WordPress 6.9 deprecation issues in plugin code | `wordpress-plugin-core` |
